...
--- Appends the "(xN)" crate-count indicator to a display description.
-- Centralized so _injectSceneCrate and _processSpawnableCrates cannot drift apart.
-- @param desc           string  base description (already i18n-translated)
-- @param cratesRequired number|nil  defaults to 1
-- @return string
local function _withCrateCountSuffix(desc, cratesRequired)
    return desc .. " (x" .. (cratesRequired or 1) .. ")"
end
...
function CTLDCrateManager:_injectSceneCrate(sceneName, model)
    ...
    local entry = {
        weight         = w,
        desc           = _withCrateCountSuffix(ctld.tr(cd.i18nKey or sceneName), cr),
        unit           = sceneName,
        side           = cd.side,
        cratesRequired = cr,
...
function CTLDCrateManager:_processSpawnableCrates()
    ...
    for category, entries in pairs(spawnableCrates) do
        ...
        -- `sc` is the raw catalogue entry (also referenced by self._weightIndex / catWeightIdx
        -- above) — mutating sc.desc in place would double-suffix it on any repeated call
        -- (this manager was already burned once on catalogue mutation, see FIX-SPEC-ISOLATION).
        -- A shallow copy carries the display-only suffix instead, leaving the catalogue pristine.
        local processedSingle = {}
        for _, sc in ipairs(singleCrates) do
            local cr = sc.cratesRequired or 1
            local displayCrate = {}
            for k, v in pairs(sc) do displayCrate[k] = v end
            displayCrate.desc = _withCrateCountSuffix(sc.desc, cr)

            local entry = { singleCrate = displayCrate }
            if cr > 1 and showCrateSets and (sc.showSets ~= false) then
                local weights = {}
                for i = 1, cr do weights[i] = sc.weight end
                entry.singleTypeSet = {
                    multiple       = weights,
                    desc           = displayCrate.desc .. allSuffix,
                    side           = sc.side,
                    isJTAC         = sc.isJTAC,
                    _autoGenerated = true,
                }
