...
if not ctld or not ctld.utils then
    ...
    _SCN_FI_ATK_RESULT = "[FI-ATK] ABORT: CTLD not initialized"
...
local function finalizeScenario()
    ...
    if S.failed == 0 then
        ...
        _SCN_FI_ATK_RESULT = TAG.." PASS "..S.passed.."/"..total
    else
        summary = TAG.." ❌ [KO] "..NAME.." — "..S.failed.." FAIL: "..
            table.concat(S.failReasons, " | ")
        _SCN_FI_ATK_RESULT = TAG.." FAIL "..S.failed.."/"..total..": "..table.concat(S.failReasons, " | ")
    end
    log(summary)
...
if not playerObjStart then
    ...
    _SCN_FI_ATK_RESULT = TAG.." ABORT: no CTLD playerObj"
    cleanup() ; return _SCN_FI_ATK_RESULT
end
...
if not menu_init then
    ...
    _SCN_FI_ATK_RESULT = TAG.." ABORT: no CTLD MenuManager menu"
    cleanup() ; return _SCN_FI_ATK_RESULT
end
