---@diagnostic disable
-- F-54 : enablePackingVehicles=false → "Pack Vehicle" absent from Crate Commands
...
ctld_test.start("F-54", "enablePackingVehicles=false — Pack Vehicle absent from Crate Commands")
...
ctld.gs = function(k)
    ...
    if k=="enablePackingVehicles"  then return false end  -- OFF
...
local playerObj = {
    unitName="UH-1H-1", groupId=9901, groupName="Grp_F54",
...
ctld_test.assert(not has({root, cc, ctld.tr("Pack Vehicle")}),      "Pack Vehicle absent (enablePackingVehicles=false)")
ctld_test.assert(has({root, cc, ctld.tr("List FOBs")}),             "List FOBs present (enabledFOBBuilding=true)")
