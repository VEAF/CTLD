...
_SCN_U107_RESULT = "[U-107] STARTED"
...
if _t.fail == 0 then
    _SCN_U107_RESULT = "[U-107] PASS " .. _t.pass .. "/" .. total
else
    local reasons = {}
    for _, line in ipairs(_t.msgs) do
        if line:sub(1, 6) == "[FAIL]" then reasons[#reasons+1] = line end
    end
    _SCN_U107_RESULT = "[U-107] FAIL " .. _t.fail .. "/" .. total .. ": " .. table.concat(reasons, "; ")
end
return _SCN_U107_RESULT
