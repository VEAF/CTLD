---@diagnostic disable
-- F-66 : canSlingload=true, transport AU SOL → menus Release/Cut absents
...
ctld_test.start("F-66", "canSlingload=true, transport au sol — menus Release/Cut absents")
...
-- Transport reported as ON GROUND
local _origInAir = ctld.utils.inAir
ctld.utils.inAir = function(_) return false end
...
local playerObj = {
    unitName="UH-1H-1", groupId=9901, groupName="Grp_F66",
...
ctld_test.assert(not has({root, cc, ctld.tr("Release Slingload")}), "Release Slingload absent (transport on ground)")
ctld_test.assert(not has({root, cc, ctld.tr("Cut Slingload")}),     "Cut Slingload absent (transport on ground)")
