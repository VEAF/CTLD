---@diagnostic disable
-- F-65 : canSlingload=false → menus Release/Cut absents même si transport en vol
...
ctld_test.start("F-65", "canSlingload=false — menus Release/Cut absents")
...
ctld.gs = function(k)
    ...
    if k=="unitActions" then
        return { ["UH-1H"] = { crates=true, troops=false, canParachute=false, canSlingload=false } }
    end
    return _origGs(k)
end
...
local playerObj = {
    unitName="UH-1H-1", groupId=9901, groupName="Grp_F65",
...
ctld_test.assert(not has({root, cc, ctld.tr("Release Slingload")}), "Release Slingload absent (canSlingload=false)")
ctld_test.assert(not has({root, cc, ctld.tr("Cut Slingload")}),     "Cut Slingload absent (canSlingload=false)")
