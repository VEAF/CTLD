---@diagnostic disable
-- F-63 : canParachute=false → menus Parachute Crates/Troops/Vehicle absents
...
ctld_test.start("F-63", "canParachute=false — menus Parachute Crates/Troops/Vehicle absents")
...
local playerObj = {
    unitName="UH-1H-1", groupId=9901, groupName="Grp_F63",
...
ctld_test.assert(not has({root, cc, ctld.tr("Parachute Crates")}),   "Parachute Crates absent (canParachute=false)")
ctld_test.assert(not has({root, tc, ctld.tr("Parachute Troops")}),   "Parachute Troops absent (canParachute=false)")
ctld_test.assert(not has({root, vc, ctld.tr("Parachute Vehicle")}),  "Parachute Vehicle absent (canParachute=false)")
-- Base sections still present
ctld_test.assert(has({root, cc}), "Crate Commands present (enableCrates=true)")
ctld_test.assert(has({root, tc}), "Troop Commands present")
ctld_test.assert(has({root, vc}), "Vehicle Commands present (canCarryVehicles=true)")
