---@diagnostic disable
-- =============================================================================
-- scenarios/scenario_jtac_vehicle_blue.lua
-- Injectable test scenario — BLUE JTAC ground vehicle (Hummer) auto-lase cycle.
...
--
-- Sequence:
--   T+0s  : cleanup ground JTACs; save enemy origin; spawn Hummer JTAC 100m from enemy
--   T+15s : VERIFY 1 — JTAC lasing enemy (currentTarget set)
--   T+45s : destroy enemy group (guaranteed target loss)
...
local VEHICLE_WEIGHT = 1001.01    -- Hummer - JTAC, side=2 (BLUE)
local VEHICLE_CTY    = country.id.USA
...
local _ok, _err = pcall(function()
    ...
    _result = string.format("started | '%s' | gname='%s' | enemy @ (%.0f,%.0f)",
