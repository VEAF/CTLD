...
local INITIAL_RADIUS = 2500 -- meters
...
for i = 1, mIdx do
    trigger.action.removeMark(i) --when markIndex removed can't be reused in the mission
end
-- ── helpers ──────────────────────────────────────────────────────────────────
local function makeCircleRoute(cx, cz, radius, n, altASL, speedMs)
    ...
    for i = 1, n do
        trigger.action.outText("i/n = " .. tostring(i), 5)
...
trigger.action.outText("Fuel % = " .. tostring(playerUnit:getFuel()), 10)

-- ── 2. Draw reference circle ──────────────────────────────────────────────────
--trigger.action.removeMark(9070)
trigger.action.circleToAll(
    -1, gidx(), pPos, INITIAL_RADIUS,
    { 0.2, 0.8, 0.2, 1.0 },
    { 0.2, 0.8, 0.2, 0.05 },
    1, false,
...
-- Draw waypoint marks
--for i = 1, NUM_PTS do
local pts = makeCircleRoute(cx, cz, INITIAL_RADIUS, NUM_PTS, altASL, speedMs)
trigger.action.outText("#pts = " .. tostring(#pts), 10)
for i = 1, #pts do
    ...
    local wPos = { x = pts[i].x, y = pts[i].alt, z = pts[i].y }
    --trigger.action.removeMark(9070 + i)
    trigger.action.markToAll(gidx(), tostring(i), wPos, false, "")
end
...
-- ── 4. Set initial route ──────────────────────────────────────────────────────
setCircleRoute(dcsGroup, cx, cz, INITIAL_RADIUS, NUM_PTS, altASL, speedMs)
