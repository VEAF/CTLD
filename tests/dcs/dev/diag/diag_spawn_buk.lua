-- diag_spawn_buk.lua
-- Spawn a complete BUK AA system (RED) 200m ahead of the player.

local AIRCRAFT_NAME = "Batumi_UH-1H_0-1"
local TEMPLATE_NAME = "BUK AA System"

local transport = Unit.getByName(AIRCRAFT_NAME)
if not transport or not transport:isExist() then
    env.info("[SPAWN_BUK] aircraft not found: " .. AIRCRAFT_NAME); return
end
...
if not template then env.info("[SPAWN_BUK] template not found"); return end
...
local hdg = ctld.utils.getHeadingInRadians("spawn_buk", transport, true)
...
local gdata = {
    ...
    ["route"] = { ["points"] = {} },
    ["groupId"] = uid, ["hidden"] = false, ["units"] = {},
    ["y"] = positions[1] and positions[1].z or origin.z,
    ["x"] = positions[1] and positions[1].x or origin.x,
    ["name"] = "CTLD_BUK_RED_" .. uid, ["start_time"] = 0,
}
for i, pos in ipairs(positions) do
    local uId = ctld.utils.getNextUniqId()
    gdata["units"][i] = {
        ["x"] = pos.x, ["y"] = pos.z, ["type"] = types[i],
        ["unitId"] = uId, ["heading"] = headings[i] or 0,
        ["playerCanDrive"] = false, ["skill"] = "Excellent",
        ["name"] = "CTLD_BUK_RED_" .. uid .. "_" .. i,
    }
end

local ok = pcall(function() coalition.addGroup(cId, Group.Category.GROUND, gdata) end)
if not ok then env.info("[SPAWN_BUK] coalition.addGroup failed"); return end

local group = Group.getByName(gdata["name"])
if not group then env.info("[SPAWN_BUK] group not found after spawn"); return end
...
env.info(string.format("[SPAWN_BUK] spawned %s → group=%s  units=%d  origin=(%.0f,%.1f,%.0f)",
