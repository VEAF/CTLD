...
# runtime; this build step only makes it available. Merged after the i18n modules.
# A long-bracket level with no colliding closer in the content is chosen dynamically.
$configDefaultLua = Join-Path $srcDir "CTLD_config_default_yaml.lua"
$yamlText = [System.IO.File]::ReadAllText($configYaml)
$eq = ""
while ($yamlText.Contains("]$eq]")) { $eq += "=" }
$open  = "[$eq["
$close = "]$eq]"
$moduleSb = [System.Text.StringBuilder]::new()
$null = $moduleSb.AppendLine('-- GENERATED by merge_CTLD.ps1 - verbatim embed of CTLD_config.yaml. DO NOT EDIT.')
$null = $moduleSb.AppendLine('ctld = ctld or {}')
$null = $moduleSb.AppendLine("ctld.configDefault = $open")
$null = $moduleSb.Append($yamlText)
$null = $moduleSb.AppendLine($close)
[System.IO.File]::WriteAllText($configDefaultLua, $moduleSb.ToString(), $utf8NoBOM)
Write-Host "Embedded CTLD_config.yaml -> ctld.configDefault ($($yamlText.Length) chars, bracket level '$eq')"
