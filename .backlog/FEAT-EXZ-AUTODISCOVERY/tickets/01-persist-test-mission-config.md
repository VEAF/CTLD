...
(`dcsZoneName` is the exact, unchanged DCS zone name — do not rename any zone in the Mission
Editor, several test scenarios look zones up by this exact string):

| `dcsZoneName` | coalition | pickup/dropoff | cargoType / aiDropMode | troopStock | vehicleStock | Used by |
|---|---|---|---|---|---|---|
| `AIZ_base_B_P_5` | BLUE | pickup | `T` | `{Standard Group: 5, Anti Tank: 2}` | — | F-176, MT-07 |
| `AIZ_front_B_D` | BLUE | dropoff | **? confirm — no test asserts a mode; `G` suggested** | — | — | MT-07 |
...
| `AIZ_mt11_B_D` | BLUE | dropoff | **? confirm — no test asserts a mode; `G` suggested** | — | — | MT-11 |
| `AIZ_mt12_B_P_V` | BLUE | pickup | `V` | *(absent)* | `{Hummer: 2}` | MT-12 |
| `AIZ_mt12_B_D` | BLUE | dropoff | **? confirm — no test asserts a mode; `G` suggested** | — | — | MT-12 |
| `AIZ_mt13_B_P_V` | BLUE | pickup | `V` | — | `{"FARP Alpha": 1}` *(scene)* | MT-13 |
| `AIZ_mt13_B_D` | BLUE | dropoff | **? confirm — no test asserts a mode; `G` suggested** | — | — | MT-13 |
| `AIZ_mt14_B_P_V` | BLUE | pickup | `V` | — | `{"HAWK AA System": 1}` *(scene)* | MT-14 |
| `AIZ_mt14_B_D` | BLUE | dropoff | **? confirm — no test asserts a mode; `G` suggested** | — | — | MT-14 |

The 5 rows marked "confirm" are dropoff zones no scenario ever checks the actual `aiDropMode` of
(only that the zone exists and that a landed AI unit's cargo empties) — `G` is a safe, consistent
default matching the only 2 confirmed dropoff zones, but a.lingo may know a different real intent
for any of them; the `ctld-tools` UI is exactly where to correct it if so.
