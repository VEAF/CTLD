# REINTEGRATE-ORPHAN-TESTS

**Status:** delivered. Compacted from `REINTEGRATE-ORPHAN-TESTS/` on 2026-08-01; the ticket files live on in git history.

Rebuilt coverage for the 46 features tested only by dead FullGas relics — 6 busted tickets (158 tests, green in CI) + 1 live-DCS ticket (3 scenarios: BCN 30/30, RCN 10/10, VEH 22/22). 10 relics were false orphans (already covered); 6 deferred to FullGas (`dev/fullgas-report.md`). PR #20.

## Tickets

| Ticket | Status | Title |
|---|---|---|
| `01-crate-lifecycle` | ✅ done | 01 — Crate lifecycle manager methods + events (PILOT) |
| `02-legacy-api-wrappers` | ✅ done | 02 — Legacy API wrappers: routing + deprecation |
| `03-scenes-minefields` | ✅ done | 03 — Scene structures + minefields |
| `04-deploy-managers` | ✅ done | 04 — Deploy managers: AA assembly, FOB events, pack vehicle |
| `05-menu-gating-player-wiring` | ✅ done | 05 — Menu gating by config + player event wiring |
| `06-jtac-recon` | ✅ done | 06 — JTAC config/deregister + recon auto-refresh |
| `07-live-dcs-coverage` | ✅ done | 07 — Live-DCS coverage (real engine required) |
