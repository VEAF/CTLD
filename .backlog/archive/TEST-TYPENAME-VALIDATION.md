...
**Status:** ✅ merged (PR #36). Compacted from `TEST-TYPENAME-VALIDATION/` on 2026-08-01; the ticket files live on in git history.

Extend `CTLDTypeCollector.collect()` to cover `aiZones[*].vehicleStock`, `capabilitiesByType[*].loadableVehiclesRED/BLUE`, and `aiZones[*].vehicleTypes` — closing the CI type-linter gap exposed by the `M1025 HMMWV Armament` silent-spawn bug.
...
| Ticket | Status | Title |
|---|---|---|
| `01-extend-type-collector` | ⬜ ready | 01 — Extend CTLDTypeCollector to cover vehicleStock, loadableVehicles, vehicleTypes |
