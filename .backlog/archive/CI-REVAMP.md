...
**Status:** delivered. Compacted from `CI-REVAMP/` on 2026-08-01; the ticket files live on in git history.

CI on `develop`, single build source, coverage ratchet (59%/61.56%), gitleaks, hygiene (PR #1).

## Tickets

| Ticket | Status | Title |
|---|---|---|
| `01-trigger-coverage-develop` | ⬜ ready | 01 — Cover `develop` on push and PR |
| `02-build-calls-merge-script` | ⬜ ready | 02 — Build job calls `merge_CTLD.ps1` (single source of truth) |
| `03-remove-broken-docs-job` | ⬜ ready | 03 — Remove the broken `docs` job (handoff to DOC-MKDOCS) |
| `04-busted-coverage-ratchet` | ⬜ ready | 04 — Busted coverage ratchet |
| `05-stylua-formatting-check` | ⬜ ready | 05 — Stylua formatting check |
| `06-gitleaks-secret-scanning` | ⬜ ready | 06 — Gitleaks secret scanning |
| `07-repo-hygiene-templates` | ⬜ ready | 07 — Repo hygiene: dependabot, CODEOWNERS, issue/PR templates |
