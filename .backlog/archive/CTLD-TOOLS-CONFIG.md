...
**Status:** ✅ merged (PR #46). Compacted from `CTLD-TOOLS-CONFIG/` on 2026-08-01; the ticket files live on in git history.

Lot 2 of `ctld-tools` ([ADR 0009](../dev/adr/0009-external-yaml-authoring-ctld-tools.md)): engine defaults moved out of `CTLD_config.lua` into `src/CTLD_config.yaml` (source of truth, sectioned mm-facing/advanced); isolated poetry package `tools/ctld-tools` (typer/ruamel/lupa, VMCT stack) with `extract` + `gen-config`; committed generated `CTLD_config_defaults.lua` + parity/drift guards + `python-quality` CI. `load()` copies `ctld.__configDefaults`.
...
| Ticket | Status | Title |
|---|---|---|
| `01-package-bootstrap` | 🧑 planned | 01 — Bootstrap the `ctld-tools` Python package |
| `02-extract-defaults-to-yaml` | 🧑 planned | 02 — One-shot extractor: `CTLD_config.lua` defaults → `ctld-config.yaml` |
| `03-gen-config-command` | 🧑 planned | 03 — `gen-config` command: `ctld-config.yaml` → Lua defaults |
| `04-roundtrip-parity-test` | 🧑 planned | 04 — Migration parity test (Lua 5.1 deep-equal) |
| `05-build-integration-and-switchover` | 🧑 planned | 05 — Build integration + switch-over |
| `06-docs-changelog-index` | 🧑 planned | 06 — Docs, CHANGELOG, ADR, backlog index |
