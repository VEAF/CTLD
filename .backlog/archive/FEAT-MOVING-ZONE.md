...
**Status:** ✅ merged (PR #49). Compacted from `FEAT-MOVING-ZONE/` on 2026-08-01; the ticket files live on in git history.

Resolve CTLD zone positions lazily via `trigger.misc.getZone()` so zones attached to a DCS Moving Zone follow their anchor unit. Covers all zone types (LGZ_, TRZ_, AIZ_, WPZ_), polygon support, anchor-death guard via `isAlive()`, `getCenter()` unified across all zone types.
...
| Ticket | Status | Title |
|---|---|---|
| `01-getcenter-unification` | ready | 01 — Add `getCenter()` to `CTLDTroopZone` + migrate direct `zone.center` accesses |
| `02-lazy-getcenter-via-dcs` | ready | 02 — Lazy `getCenter()` via `trigger.misc.getZone()` for LGZ_ and TRZ_ |
| `03-anchor-detection-and-isalive` | ready | 03 — Moving Zone anchor detection at init + `isAlive()` / `isDynamic()` extension |
| `04-polygon-relative-offsets` | ready | 04 — Polygon Moving Zone: relative-offset vertex reconstruction |
| `05-l3-dcs-scenario` | ready | 05 — L3 DCS noPlayer scenario: Moving Zone live position tracking |
