...
**Status:** ✅ merged (PR #57). Compacted from `FIX-I18N-DICT-SYNC/` on 2026-08-01; the ticket files live on in git history.

Fix `$repoRoot` path bug in `generate_i18n_dicts.ps1` (1 level too shallow → looks in `tools/src/`, finds 0 keys); run `-Apply` to sync missing keys; fill FR translations for 60+ missing menu labels.
...
| Ticket | Status | Title |
|---|---|---|
| `01-fix-reporoot-and-sync-dicts` | open | Ticket 01 — Fix repoRoot bug and sync all dictionaries with FR translations |
