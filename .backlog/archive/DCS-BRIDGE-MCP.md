...
**Status:** delivered. Compacted from `DCS-BRIDGE-MCP/` on 2026-08-01; the ticket files live on in git history.

Migrate DCS integration testing from Witchcraft to VEAF-dcs-bridge: `.mcp.json` + project-local venv, scenario return contract, 79 scenarios + 4 templates migrated, `integration-testing` skill, Witchcraft fully retired (PR #14). Uncovered ~194 dead FullGas relics → `CLEANUP-LEGACY-DCS-TESTS`.

## Tickets

| Ticket | Status | Title |
|---|---|---|
| `01-mcp-json-wiring` | ✅ done | 01 — Wire `.mcp.json` + dcs-client config |
| `02-return-contract` | ✅ done | 02 — Freeze the scenario return contract |
| `03-migrate-templates` | ✅ done | 03 — Migrate the four scenario templates |
| `04-bigbang-migrate-scenarios` | ✅ done (79 sane scenarios migrated — see PRD note on the 194 dead relics excluded) | 04 — Big-bang migrate the ~273 scenarios |
| `05-integration-testing-skill` | ✅ done | 05 — `integration-testing` skill (replaces the Witchcraft workflow) |
| `06-remove-witchcraft` | ✅ done (also swept tests/ci/, tests/dcs/util/, tests/dcs/dev/, tests/manual_test_sequences.md — beyond original scope, approved) | 06 — Remove Witchcraft from the tooling |
| `07-dev-setup-script` | ✅ done — unblocked by VEAF-dcs-bridge LOT-012 (PR #15, merged to develop, v0.6.2) | 07 — dev-setup script for dcs-bridge (project-local venv) |
