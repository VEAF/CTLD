...
**Status:** ✅ merged (PR #56). Compacted from `STARTUP-REPORT-UNIFIED/` on 2026-08-01; the ticket files live on in git history.

Unified startup report: `ctld.startupReport` collector in `CTLD_utils`, `flush()` at end of `ctld.initialize()`, two severity levels (ERROR/NOTICE), always writes `CTLD_STARTUP_REPORT` to `DCS.log`, single screen `outText` when issues exist. ADR 0010 for two-family separation.
...
| Ticket | Status | Title |
|---|---|---|
| `01-collector-and-flush-skeleton` | 🧑 planned | 01 — Collector + flush() skeleton |
| `02-migrate-crate-zone-managers` | 🧑 planned | 02 — Migrate CrateManager + ZoneManager to startup report |
| `03-migrate-usersetup-core-adr0010` | 🧑 planned | 03 — Migrate UserSetup + CoreManager + ADR 0010 |
| `04-dcs-integration-scenario` | 🧑 planned | 04 — DCS integration scenario |
