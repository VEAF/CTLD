# TEST-PLUGIN-POSTINIT

**Status:** ✅ merged (PR #31). Compacted from `TEST-PLUGIN-POSTINIT/` on 2026-08-01; the ticket files live on in git history.

L3 `noPlayer` `auto` scenario (F-124) verifying the post-init plugin contract: `registerSceneModel` after init, `deferMenuSection` direct routing (not queued), `requiresCtld` soft-fail. PASS 7/7.
...
| Ticket | Status | Title |
|---|---|---|
| `01-scenario-f124-postinit` | ⬜ ready | 01 — Implement F-124 post-init plugin scenario |
