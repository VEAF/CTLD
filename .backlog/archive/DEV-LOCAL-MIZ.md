...
**Status:** ✅ merged (PR #40). Compacted from `DEV-LOCAL-MIZ/` on 2026-08-01; the ticket files live on in git history.

Kill the hardcoded `CTLD.lua` path in the shared martyr miz: MISSION START trigger loads via `CTLD_DEV_ROOT` env var (de-sanitized DCS), hardened with explicit on-screen failure; delete the dead `ctldLogPath` line. Relocated the live-DCS testing page into `docs/developer/` + martyr setup section + realigned `dcs-runtime-debug`. Stops the committed binary miz from carrying machine paths.

## Tickets

| Ticket | Status | Title |
|---|---|---|
| `01-miz-trigger-env-var` | ✅ done | 01 — Replace the hardcoded miz path with the CTLD_DEV_ROOT trigger |
| `02-dev-setup-doc-and-skill` | ✅ done | 02 — Dev-setup checklist + realign the dcs-runtime-debug skill |
