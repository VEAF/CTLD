...
**Status:** ✅ merged (PR #30). Compacted from `FIX-AI-C2-BUGS/` on 2026-08-01; the ticket files live on in git history.

Two bugs in the AI transport C2 (virtual stock) path: wrong activation guard when a physical vehicle is weight-rejected (Bug 1), and invalid typeName `M1025 HMMWV Armament` in example config causing silent Leopard-2 spawn (Bug 2a/2b). MT-08B PASS 7/7.

## Tickets

| Ticket | Status | Title |
|---|---|---|
| `01-fix-c2-guard` | ✅ done | 01 — Fix C2 activation guard in `onAILand` |
| `02-fix-invalid-typename-config` | ✅ done | 02 — Replace invalid typeName in virtual stock example config |
| `03-validate-typename-at-load` | 🚫 wontfix | 03 — Validate vehicleStock typeNames at zone-load time |
| `04-mt08b-regression-pass` | ✅ done | 04 — MT-08B regression: verify PASS after all C2 fixes |
