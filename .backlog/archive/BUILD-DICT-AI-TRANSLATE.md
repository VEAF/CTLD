# BUILD-DICT-AI-TRANSLATE

**Status:** ✅ merged (PR #60). Compacted from `BUILD-DICT-AI-TRANSLATE/` on 2026-08-01; the ticket files live on in git history.

Traduire automatiquement les stubs i18n vides via l'API Claude Haiku (local, si `ANTHROPIC_API_KEY` dispo) + câbler `ctld.i18n_auditAll()` dans `ctld.startupReport` pour signaler les clés vides au runtime.
...
| Ticket | Status | Title |
|---|---|---|
| `01-i18n-startup-report-wiring` | ready | 01 — i18n startup-report wiring (Lua) |
| `02-translate-i18n-py` | ready | 02 — translate_i18n.py — script de traduction locale |
| `03-merge-ctld-integration` | ready | 03 — Intégration merge_CTLD.ps1 |
