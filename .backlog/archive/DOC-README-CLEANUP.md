...
**Status:** ✅ merged (PR #21). Compacted from `DOC-README-CLEANUP/` on 2026-08-01; the ticket files live on in git history.

Fix README title, restructure Crate Operations sub-sections, relocate AA System Construction, replace Developer Guide with Documentation links.

## Tickets

| Ticket | Status | Title |
|---|---|---|
| `01-rename-h1-title` | done | 01 — Rename the README title from `DCS-CTLD Next` to `CTLD` |
| `02-replace-developer-guide-with-documentation-links` | done | 02 — Replace `Developer Guide` with a `Documentation` quick-links section |
| `03-restructure-crate-operations-cluster` | done | 03 — Restructure the Crate Operations cluster |
