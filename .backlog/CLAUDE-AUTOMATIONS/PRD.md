...
`CTLD_Next.lua`. It is easy to violate them by reflex. There is also no specialized helper to
...
- Hook logic is verifiable by piping representative tool-call JSON to the script and asserting the
  ...
  on names like `src/..CTLD_Next..lua`. This was run locally before wiring.
...
user to review/trust project hooks on first use. The build (`merge_CTLD.ps1`) writes `CTLD_Next.lua`
