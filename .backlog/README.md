...
| `FIX-I18N-DICT-SYNC` | 🔄 in progress | Fix `$repoRoot` path bug in `generate_i18n_dicts.ps1` (1 level too shallow → looks in `tools/src/`, finds 0 keys); run `-Apply` to sync missing keys; fill FR translations for 60+ missing menu labels. | `fix/i18n-dict-sync` |
