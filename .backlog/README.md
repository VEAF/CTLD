...
|-----|--------|-------------|--------|
| `BUILD-DICT-AI-TRANSLATE` | open | Traduire automatiquement les stubs i18n vides via l'API Claude Haiku (local, si `ANTHROPIC_API_KEY` dispo) + câbler `ctld.i18n_auditAll()` dans `ctld.startupReport` pour signaler les clés vides au runtime. | `feature/build-dict-ai-translate` |
