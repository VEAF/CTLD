...
| `STARTUP-REPORT-UNIFIED` | ⬜ ready | Unified startup report: `ctld.startupReport` collector in `CTLD_utils`, `flush()` at end of `ctld.initialize()`, two severity levels (ERROR/NOTICE), always writes `CTLD_STARTUP_REPORT` to `DCS.log`, single screen `outText` when issues exist. ADR 0010 for two-family separation. | — |
