...
| `FIX-AI-C2-BUGS` | ✅ pending merge | Two bugs in the AI transport C2 (virtual stock) path: wrong activation guard when a physical vehicle is weight-rejected (Bug 1), and invalid typeName `M1025 HMMWV Armament` in example config causing silent Leopard-2 spawn (Bug 2a/2b). MT-08B PASS 7/7. PR #30. | `fix/ai-c2-bugs` |
