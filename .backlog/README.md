...
| `CTLD-TOOLS-CORE` | 🚧 in progress | Lot 2/3 — UI-agnostic tool core: demolish ops model/TUI/tkinter/`reference.json`/`gen-config`; complete-catalogue load/edit/save + `validate` (schema + datamine + mixedSet) + version-gap diff; keep `miz`-inject + `datamine`; CLI trimmed to build/CI needs. Ships a **library**, no new UI. | `feature/ctld-tools-core` |
